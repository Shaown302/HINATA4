...
async def cmd_copilot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt:
        ...
        await update.effective_message.reply_text("💡 <b>AI Copilot:</b>\n\nWhat should I think deeply about?", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="btn_back")]]))
        return
    msg = await update.effective_message.reply_text("💡 <b>Thinking Deeply...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
...
async def cmd_pinterest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        await update.effective_message.reply_text("📌 <b>Usage:</b> <code>/pinterest [query]</code>", parse_mode="HTML")
        return
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if media and isinstance(media, list):
                    # Just send first 5 to avoid flooding
                    for i in range(min(5, len(media))):
                        img_url = media[i] if isinstance(media[i], str) else media[i].get("url")
                        if img_url:
                            await download_and_send_photo(client, img_url, update, "")
                    await msg.delete()
                    return
            await msg.edit_text("❌ No images found on Pinterest for this query.")
        except Exception as e:
            await msg.edit_text(f"❌ Pinterest Error: {e}")

async def cmd_wallpaper_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_permission(update, context): return
    query = " ".join(context.args)
    if not query:
        await update.effective_message.reply_text("🖼 <b>Usage:</b> <code>/wallpaper [query]</code>", parse_mode="HTML")
        return
    msg = await update.effective_message.reply_text("🖼 <b>Searching Wallpapers...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                if media and isinstance(media, list):
                    ...
                    if isinstance(img_url, dict): img_url = img_url.get("image") or img_url.get("url")
                    if img_url:
                        success = await download_and_send_photo(client, img_url, update, "✨ <b>Wallpaper Found!</b>")
                        if success:
                            await msg.delete()
                            return
            await msg.edit_text("❌ No wallpapers found.")
        except Exception as e:
...
async def cmd_ytsearch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = " ".join(context.args)
    if not query:
        await update.effective_message.reply_text("🎬 <b>Usage:</b> <code>/ytsearch [query]</code>", parse_mode="HTML")
        return
    msg = await update.effective_message.reply_text("🎬 <b>Searching YouTube...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                if results and isinstance(results, list):
                    text = "🎬 <b>YouTube Search Results</b>\n<i>Click a button to download!</i>\n\n"
                    kb = []
                    for i, res in enumerate(results[:5]):
                        title = res.get("title", f"Video {i+1}")
                        url = res.get("url")
                        text += f"<b>{i+1}.</b> <a href='{url}'>{html.escape(title)}</a>\n"
                        if url:
                            kb.append([InlineKeyboardButton(f"📥 Download {i+1}", callback_data=f"yt_dl_req|{url}")])
                    ...
                    return
            await msg.edit_text("❌ No results found on YouTube.")
        except Exception as e:
            await msg.edit_text(f"❌ YouTube Search Error: {e}")
...
async def cmd_styletext(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.effective_message.reply_text("✨ <b>Usage:</b> <code>/styletext [text]</code>", parse_mode="HTML")
        return
    msg = await update.effective_message.reply_text("✨ <b>Styling Text...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(STYLE_TEXT_API.format(text=quote(text)), timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
                res = data.get("styles") or data.get("result") or data.get("data")
                if res and isinstance(res, list):
                    reply = "✨ <b>Styled Text Options:</b>\n\n"
                    for s in res[:20]:
                        val = s.get('styled_text') if isinstance(s, dict) else s
                        if val: reply += f"<code>{html.escape(val)}</code>\n\n"
                    await msg.edit_text(reply, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="btn_back")]]))
                    return
            await msg.edit_text("❌ Could not style text.")
        except Exception as e:
            await msg.edit_text(f"❌ Styling Error: {e}")
...
WALLPAPER_SEARCH_API = "https://apis.prexzyvilla.site/search/wallpaper?query={query}&page%3F="
...
async def do_textmaker_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(VISUAL_TEXT_STYLES), 2):
        row = [
            InlineKeyboardButton(VISUAL_TEXT_STYLES[i].replace("-", " ").title(), callback_data=f"txtstyle_{VISUAL_TEXT_STYLES[i]}"),
            InlineKeyboardButton(VISUAL_TEXT_STYLES[i+1].replace("-", " ").title(), callback_data=f"txtstyle_{VISUAL_TEXT_STYLES[i+1]}") if i+1 < len(VISUAL_TEXT_STYLES) else None
        ]
        kb.append([b for b in row if b])
...
async def log_bot_response(update: Update, context: ContextTypes.DEFAULT_TYPE, response_text: str):
    try:
        ...
        log_text = (
            f"📤 <b>Bot Response Log</b> 📤\n\n"
            f"👤 <b>To User:</b> {from_info} (<code>{user.id}</code>)\n"
            f"🏘 <b>In Chat:</b> {html.escape(chat_title)} (<code>{chat.id}</code>)\n"
            f"📝 <b>Response:</b>\n<i>{html.escape(response_text[:3000])}</i>"
        )
...
async def fetch_deepseek(client: httpx.AsyncClient, prompt: str):
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            if data.get("status") or isinstance(data, dict):
                # Try multiple possible response keys based on common API structures
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data.get("status") or isinstance(data, dict):
                res = data.get("result") or data.get("data", "") or data.get("response", "") or data.get("answer", "")
                if res:
                    return f"💡 <b>Copilot (Think Deeper):</b>\n\n{res.strip()}"
            return "⚠️ Copilot declined the request."
        return f"💡 Copilot Error {resp.status_code}: Service is currently resting."
    except Exception as e:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        f"🌌 <b>WELCOME TO HINATA CORE v3.0</b> 🌌\n"
        f"────────────────────────\n"
        f"👋 <b>Greetings, {html.escape(first_name)}!</b>\n"
        f"I am <b>Hinata</b>, your elite neural companion. I am engineered to provide high-fidelity AI processing, global media logistics, and advanced interactive systems.\n\n"
        f"💎 <b>CORE ARCHITECTURE:</b>\n"
        f"• 🧠 <b>Neural Engines:</b> Gemini 3.0, DeepSeek R1, GPT-5\n"
        f"• 🎨 <b>Visual Synthesis:</b> Imagine Art & Textmaker Pro\n"
        f"• 📥 <b>Data Logistics:</b> Universal Multi-Platform DL\n"
        f"• ✉️ <b>Secure Comms:</b> Anonymous Email & WebSS\n"
        f"• 🔍 <b>AI Help Matrix:</b> <code>/help [topic]</code>\n\n"
        f"✨ <i>Powered by Shawon Intelligence</i>"
    )
    ...
    if category == "home":
        kb = [
            ...
            [InlineKeyboardButton("👤 DEVELOPER", url="https://t.me/ShawonXnone"),
             InlineKeyboardButton("📊 SYSTEM DASH", url=f"http://localhost:10000")] # Fallback URL
        ]
        ...
        kb = [
            ...
            [InlineKeyboardButton("🌌 Imagine Art", callback_data="btn_imagine"),
             InlineKeyboardButton("🎨 Text Visualizer", callback_data="btn_textmaker")],
        ...
        kb = [
            ...
            [InlineKeyboardButton("📌 Pinterest", callback_data="btn_pinterest_req"),
             InlineKeyboardButton("🎬 YT Search", callback_data="btn_ytsearch_req")],
            [InlineKeyboardButton("🖼 Wallpapers", callback_data="btn_wallpaper"),
             InlineKeyboardButton("🔍 Wall Search", callback_data="btn_wallsearch_req")],
            [InlineKeyboardButton("🔙 Back", callback_data="btn_back")]
        ]
        return InlineKeyboardMarkup(kb)
        ...
        kb = [
            [InlineKeyboardButton("❌ TTT ⭕", callback_data="btn_game_ttt"),
             InlineKeyboardButton("❓ Riddle", callback_data="btn_game_riddle")],
            [InlineKeyboardButton("🎲 Trivia", callback_data="btn_game_trivia"),
             InlineKeyboardButton("🤫 Truth/Dare", callback_data="btn_game_tod")],
            [InlineKeyboardButton("🎮 FF Stalk", callback_data="btn_ff_stalk_req"),
             InlineKeyboardButton("🎰 Slot Machine", callback_data="btn_game_slot")],
            [InlineKeyboardButton("🔙 Back", callback_data="btn_back")]
        ]
        return InlineKeyboardMarkup(kb)
...
def clear_states(ud):
    ...
    for key in [AWAIT_GEMINI, AWAIT_DEEPSEEK, AWAIT_FLIRT, AWAIT_INSTA, AWAIT_USERINFO, AWAIT_FF, AWAIT_CODE, AWAIT_DL, AWAIT_QRGEN, AWAIT_BGREM, AWAIT_TRANSLATE, AWAIT_SUMMARIZE, AWAIT_GRAMMAR, AWAIT_BAN, AWAIT_UNBAN, AWAIT_GUESS, AWAIT_RIDDLE, AWAIT_HINATA, AWAIT_FFVISIT, AWAIT_TRIVIA]:
        ud.pop(key, None)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_gemini":
        ...
        await safe_edit(query, "✨ <b>Style Text Matrix</b>\n\nUsage: <code>/styletext [text]</code>", reply_markup=back)
...
async def handle_dl_cmd(u, c):
    ...
    else: await u.effective_message.reply_text("📥 <b>Hinata Downloader</b>\n\nUsage: <code>/dl [URL]</code>\n\n<i>Supported: YT, IG, TikTok, Twitter, FB, and many more!</i>", parse_mode="HTML")

async def handle_insta_cmd(u, c):
    if not await check_permission(u, c): return
    clear_states(c.user_data)
    if c.args: await do_insta_fetch_by_text(u, c, c.args[0])
    else: await u.effective_message.reply_text(" <b>Usage:</b> <code>/insta [username]</code>", parse_mode="HTML")

async def handle_ff_cmd(u, c):
    if not await check_permission(u, c): return
    clear_states(c.user_data)
    if c.args: await do_ff_fetch_by_text(u, c, c.args[0])
    else: await u.effective_message.reply_text(" <b>Usage:</b> <code>/ff [uid]</code>", parse_mode="HTML")
...
async def start_bot():
    ...
    try:
        ...
        app.add_handler(CommandHandler("ffstalk", cmd_ffstalk))
        app.add_handler(CommandHandler("ff", cmd_ffstalk))
